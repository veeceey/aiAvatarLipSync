...
//     const expressionTargetsRef = useRef([]);
//     const animationFrameRef = useRef(null);
  
//     useEffect(() => {
//       // Detect head, mouth, and expression morph targets
...
//             ]
//             .filter(targetName => object.morphTargetDictionary[targetName] !== undefined)
//             .map(targetName => ({
//               name: targetName,
//               index: object.morphTargetDictionary[targetName]
//             }));
  
//             // Expression targets
//             const expressionTargets = [
//               'mouthSmile', 'mouthFrownLeft', 'mouthFrownRight',
//               'mouthShrugUpper', 'mouthPucker',
...
//               console.log('Mouth targets:', mouthTargets.map(t => t.name));
//             }
  
//             if (expressionTargets.length > 0) {
//               expressionTargetsRef.current = expressionTargets;
//               console.log('Expression targets:', expressionTargets.map(t => t.name));
//             }
//           }
//         }
//       });
...
//       const mouthTargets = mouthTargetsRef.current;
//       const expressionTargets = expressionTargetsRef.current;
...
//       if (!speaking || !currentHead || mouthTargets.length === 0) {
//         // Smooth reset of morph targets
//         [...mouthTargets, ...expressionTargets].forEach(({ index }) => {
//           if (currentHead.morphTargetInfluences) {
//             let currentValue = currentHead.morphTargetInfluences[index];
            
//             const resetAnimation = () => {
//               currentValue = Math.max(0, currentValue - 0.1);
//               currentHead.morphTargetInfluences[index] = currentValue;
...
//       // Continuous mouth and expression animation
//       let frameCount = 0;
...
//         // Mouth movement
//         const mouthIntensity = Math.sin(frameCount * 0.2) * 0.4;
//         mouthTargets.forEach(({ index, name }) => {
//           if (currentHead.morphTargetInfluences) {
//             const isOpenMouth = 
//               name.includes('Open') || 
//               name.includes('viseme_');
            
//             const value = isOpenMouth ? Math.abs(mouthIntensity) : 0;
...
//         expressionTargets.forEach(({ index, name }) => {
//           if (currentHead.morphTargetInfluences) {
...
//             currentHead.morphTargetInfluences[index] = expressionIntensity;
//           }
//         });
...
const Avatar = ({ speaking, emotion = 'neutral' }) => {
    ...
    const MOUTH_INTENSITY = 0.1; // Lower value for subtle mouth movement
    const MAX_MOUTH_OPEN = 0.2; // Maximum mouth opening
    ...
    useEffect(() => {
      ...
      // Calculate scale factor to fit within a reasonable size
      ...
      scene.position.y = -0.5; // Adjust vertical positioning
      scene.position.z = -0; // Move slightly back
      scene.rotation.y = 0; // Rotate 180 degrees if needed
    ...
    // Existing lip-sync and expression animation useEffect remains the same
    useEffect(() => {
      const currentHead = headRef.current;
      const mouthTargets = mouthTargetsRef.current;
      const expressionTargets = expressionTargetsRef.current;
      ...
      const animate = () => {
        ...
        // Mouth movement with controlled intensity
        const mouthIntensity = Math.sin(frameCount * 0.2) * MOUTH_INTENSITY;
        mouthTargets.forEach(({ index, name }) => {
          if (currentHead.morphTargetInfluences) {
            const isOpenMouth = 
              name.includes('Open') || 
            ...
            const value = isOpenMouth ? Math.abs(mouthIntensity) : 0;
            currentHead.morphTargetInfluences[index] = 
              Math.max(0, Math.min(MAX_MOUTH_OPEN, value));
          }
        });
  
        // Vowel-specific mouth movements
        const vowelTargets = mouthTargets.filter(target => 
          ['viseme_aa', 'viseme_E', 'viseme_I', 'viseme_O', 'viseme_U'].includes(target.name)
        );
  
        vowelTargets.forEach(({ index, name }) => {
          if (currentHead.morphTargetInfluences) {
            let vowelIntensity = 0;
            
            // Different intensities for different vowel sounds
            switch(name) {
              case 'viseme_aa': // 'A' sound
                vowelIntensity = MOUTH_INTENSITY * 0.8;
                break;
              case 'viseme_E': // 'E' sound
                vowelIntensity = MOUTH_INTENSITY * 0.6;
                break;
              case 'viseme_I': // 'I' sound
                vowelIntensity = MOUTH_INTENSITY * 0.4;
                break;
              case 'viseme_O': // 'O' sound
                vowelIntensity = MOUTH_INTENSITY * 0.7;
                break;
              case 'viseme_U': // 'U' sound
                vowelIntensity = MOUTH_INTENSITY * 0.5;
                break;
            }
  
            currentHead.morphTargetInfluences[index] = 
              Math.max(0, Math.min(MAX_MOUTH_OPEN, vowelIntensity));
          }
        });
